...
if (isset($_POST['action']))
{
	if ($_POST['action'] == "edit"){
		$stonescheduleid = isset($_POST['stonescheduleid']) ? $_POST['stonescheduleid'] : die('ERROR: Schedule ID not found.');

		if ($stmt = $mysqli->prepare("SELECT StoneScheduleID, UserID, JobID, Description, ScheduleDate, SortOrder, Notes FROM tblStoneSchedule WHERE StoneScheduleID = ? LIMIT 1")) { 
			$stmt->bind_param('i', $stonescheduleid);
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($formstonescheduleid, $formuserid, $formjobid, $formdescription, $formscheduledate, $formsortorder, $formnotes);
			$stmt->fetch();
		}
	}
}
...
<form id='schedule-form' action='#' method='post'>
    ...
    <input type="hidden" id="stonescheduleid" name="stonescheduleid" value="<?php if (isset($_POST['stonescheduleid'])) { echo $_POST['stonescheduleid']; } ?>">
	...
	<div class="form-group">
		...
		<select id="inputUserID" name="inputUserID" class="form-control" required>
			...
			<?php
				$query = "SELECT UserID, FullName FROM tblUser WHERE IsStone <> 0 AND IsForeman = 0 AND Active <> 0 ORDER BY FullName";
	...
	<button type="button" id="delete-btn" class="btn btn-danger pull-right" value="<?php if (isset($formstonescheduleid)) { echo $formstonescheduleid; } ?>"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Delete</button> 
	<?php } ?>
</form>