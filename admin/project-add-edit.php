...
if (isset($_POST['action']))
{
	if ($_POST['action'] == "edit"){
		...
		if ($stmt = $mysqli->prepare("SELECT ProjectID, ProjectName, Discription FROM tblproject WHERE ProjectID = ? LIMIT 1")) { 
			$stmt->bind_param('i', $projectid);
			$stmt->execute();	
			$stmt->store_result();
			$stmt->bind_result($formprojectid, $formprojectname, $formprojectDiscription);
			$stmt->fetch();
		}
	}
}
