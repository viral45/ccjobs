                ...
                <tr>
                    ...
                    <?php 
                        foreach ($datearray as $day){ 
                            ...
                            $schedulequery = "SELECT tblStoneSchedule.StoneScheduleID, tblJob.JobAddress, tblJob.JobID, tblStoneSchedule.Description FROM tblJob RIGHT JOIN tblStoneSchedule ON tblJob.JobID = tblStoneSchedule.JobID WHERE ScheduleDate = '" . date('Y-m-d',strtotime($day)) . "' AND UserID = '" . $row['UserID'] ."' ORDER BY SortOrder";
                            ...
                            while($schedulerow = $scheduleresult->fetch_array()){
                                if (!empty($schedulerow['JobID'])){
                                    ...
                                    echo "<div class='alert alert-warning calendar-entry' data-action='edit' data-schedule-id='" . $schedulerow['StoneScheduleID'] . "'><button type='button' class='close delete-btn' aria-label='Close' value='" . $schedulerow['StoneScheduleID'] . "'><span aria-hidden='true'>&times;</span></button>" . $alertstring . " " . $schedulerow['JobAddress'] . "</div>";
