...
if (!empty($_REQUEST['searchDate'])){	
	...
	$where .= " AND tblJobMaterial.CNCDateCompleted BETWEEN '" . date("Y-m-d", strtotime($dates[0])) . "' AND '" . date("Y-m-d", strtotime($dates[1] . '+1 day')) . "'";
}

if (!empty($_REQUEST['searchCompletedBy'])){	
	$where .= " AND tblJobMaterial.CNCCompletedBy = '" . $_REQUEST['searchCompletedBy'] . "'";
}
...
<div id="print-content">
    ...
    <h1>
        <span id="pageTitle">CNC Jobs (completed)</span>
    ...
    <div class="table">
        ...
        <?php
			$query = "SELECT Count(tblJob.JobID) FROM tblJob INNER JOIN tblJobMaterial ON tblJob.JobID = tblJobMaterial.JobID $where AND tblJobMaterial.CNCDateCompleted IS NOT NULL AND tblJobMaterial.BoardQuantity <> 0 ORDER BY tblJob.JobID";
        ...
        ?> 
            ...
            <table class="table table-striped">
                ...
                <tbody>
                	...
                	$query = "SELECT tblJob.JobID, tblJob.JobAddress, tblJobMaterial.BoardDescription, tblJobMaterial.BoardType, tblJobMaterial.BoardQuantity, tblJobMaterial.CNCNotes,  tblJobMaterial.CNCDateCompleted, tblUser.FullName FROM (tblJob INNER JOIN tblJobMaterial ON tblJob.JobID = tblJobMaterial.JobID) INNER JOIN tblUser ON tblJobMaterial.CNCCompletedBy = tblUser.UserID $where AND tblJobMaterial.CNCDateCompleted IS NOT NULL AND tblJobMaterial.BoardQuantity <> 0 ORDER BY $sortorder";
					$result = $mysqli->query($query);
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?php if (!empty($row['CNCDateCompleted'])){echo date("d-m-Y", strtotime($row['CNCDateCompleted'])) . " by " . $row['FullName']; } ?></td>
