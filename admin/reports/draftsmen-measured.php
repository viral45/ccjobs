...
<div id="print-content">
    ...
    <h1>
        <span id="pageTitle">Draftsmen Jobs (measured)</span>
    ...
    <div class="table">
        ...
        <?php
			$query = "SELECT Count(tblJob.JobID) FROM tblJob INNER JOIN tblJobTaskDraft ON tblJob.JobID = tblJobTaskDraft.JobID $where AND tblJobTaskDraft.DateStarted IS NOT NULL AND tblJobTaskDraft.DateDrawn IS NULL ORDER BY tblJob.JobID";
        ...
        ?> 
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    <?php 
                	$query = "SELECT tblJob.JobID, tblJob.JobAddress, tblTask.TaskName, tblJobTaskDraft.DateStarted, tblUser.FullName FROM ((tblJob INNER JOIN tblJobTaskDraft ON tblJob.JobID = tblJobTaskDraft.JobID) INNER JOIN tblTask ON tblJobTaskDraft.TaskID = tblTask.TaskID) INNER JOIN tblUser ON tblJobTaskDraft.StartedBy = tblUser.UserID $where AND tblJobTaskDraft.DateStarted IS NOT NULL AND tblJobTaskDraft.DateDrawn IS NULL ORDER BY $sortorder";
					$result = $mysqli->query($query);
