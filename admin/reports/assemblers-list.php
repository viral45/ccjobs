...
if (!empty($_REQUEST['searchDate'])){	
	...
	$where .= " AND tblJobTask.DateStarted BETWEEN '" . date("Y-m-d", strtotime($dates[0])) . "' AND '" . date("Y-m-d", strtotime($dates[1] . '+1 day')) . "'";
}
...
<div id="print-content">
    ...
    <h1>
        <span id="pageTitle">Assembler Jobs</span>
    ...
    <div class="table">
        ...
        <?php
			$query = "SELECT Count(tblJob.JobID) FROM tblJob INNER JOIN tblJobTask ON tblJob.JobID = tblJobTask.JobID $where AND tblJobTask.DateStarted IS NOT NULL";
