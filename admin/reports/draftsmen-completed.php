...
if (!empty($_REQUEST['searchDate'])){	
	...
	$where .= " AND tblJobTaskDraft.DateCompleted BETWEEN '" . date("Y-m-d", strtotime($dates[0])) . "' AND '" . date("Y-m-d", strtotime($dates[1] . '+1 day')) . "'";
}
...
<div id="print-content">
    ...
    <h1>
        <span id="pageTitle">Draftsmen Jobs (sent to CNC)</span>
    ...
    <div class="table">
        ...
        <?php
			$query = "SELECT Count(tblJob.JobID) FROM tblJob INNER JOIN tblJobTaskDraft ON tblJob.JobID = tblJobTaskDraft.JobID $where AND tblJobTaskDraft.DateCompleted IS NOT NULL ORDER BY tblJob.JobID";
