...
if (isset($_POST['action'])){	
	...
	if ($_POST['action'] == "check"){
		...
		if ($update_stmt->affected_rows != -1){
			...
			$jobquery = "SELECT JobID, JobAddress FROM tblJob WHERE JobID = $jobid";
        	$jobresult = $mysqli->query($jobquery);
        	$jobrow = $jobresult->fetch_array();

			$email_subject = "Job #" . $jobid . " - " . $jobrow['JobAddress'] . " is ready to be checked";
			$email_message = "Job #" . $jobid . " - " . $jobrow['JobAddress'] . " is ready to be checked<br><br><a href='http://www.challengecabinetsjobs.com.au'>www.challengecabinetsjobs.com.au</a>";
	...
	if ($_POST['action'] == "savechecklist"){
		if ($_SESSION['is_foreman'] <> 0){
			...
			$missingitems = $_POST['inputMissingItems'];
