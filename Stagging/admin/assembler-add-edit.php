...
if (isset($_POST['action']))
{
	if ($_POST['action'] == "edit"){
		$assemblerscheduleid = isset($_POST['assemblerscheduleid']) ? $_POST['assemblerscheduleid'] : die('ERROR: Schedule ID not found.');

		if ($stmt = $mysqli->prepare("SELECT AssemblerScheduleID, UserID, JobID, Description, ScheduleDate, SortOrder, Notes FROM tblAssemblerSchedule WHERE AssemblerScheduleID = ? LIMIT 1")) { 
			$stmt->bind_param('i', $assemblerscheduleid);
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($formassemblerscheduleid, $formuserid, $formjobid, $formdescription, $formscheduledate, $formsortorder, $formnotes);
			$stmt->fetch();
		}
	}
}
...
<form id='schedule-form' action='#' method='post'>
    ...
    <input type="hidden" id="assemblerscheduleid" name="assemblerscheduleid" value="<?php if (isset($_POST['assemblerscheduleid'])) { echo $_POST['assemblerscheduleid']; } ?>">
	...
	<div class="form-group">
		...
		<select id="inputUserID" name="inputUserID" class="form-control" required>
			...
			<?php
				$query = "SELECT UserID, FullName FROM tblUser WHERE IsAssembler <> 0 AND IsForeman = 0 AND Active <> 0 ORDER BY FullName";
	...
	<button type="button" id="delete-btn" class="btn btn-danger pull-right" value="<?php if (isset($formassemblerscheduleid)) { echo $formassemblerscheduleid; } ?>"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Delete</button> 
	<?php } ?>
</form>