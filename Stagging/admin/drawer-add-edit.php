...
if (isset($_POST['action']))
{
	if ($_POST['action'] == "edit"){
		$drawerscheduleid = isset($_POST['drawerscheduleid']) ? $_POST['drawerscheduleid'] : die('ERROR: Drawer Schedule ID not found.');

		if ($stmt = $mysqli->prepare("SELECT DrawerScheduleID, UserID, JobID, Description, ScheduleDate, SortOrder, Notes FROM tblDrawerSchedule WHERE DrawerScheduleID = ? LIMIT 1")) { 
			$stmt->bind_param('i', $drawerscheduleid);
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($formdrawerscheduleid, $formuserid, $formjobid, $formdescription, $formscheduledate, $formsortorder, $formnotes);
			$stmt->fetch();
		}
	}
}
...
<form id='schedule-form' action='#' method='post'>
    ...
    <input type="hidden" id="drawerscheduleid" name="drawerscheduleid" value="<?php if (isset($_POST['drawerscheduleid'])) { echo $_POST['drawerscheduleid']; } ?>">
	...
	<div class="form-group">
		...
		<select id="inputUserID" name="inputUserID" class="form-control" required>
			...
			<?php
				$query = "SELECT UserID, FullName FROM tblUser WHERE IsDraftsman <> 0 AND IsForeman = 0 AND Active <> 0 ORDER BY FullName";
	...
	<button type="button" id="delete-btn" class="btn btn-danger pull-right" value="<?php if (isset($formdrawerscheduleid)) { echo $formdrawerscheduleid; } ?>"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Delete</button> 
	<?php } ?>
</form>