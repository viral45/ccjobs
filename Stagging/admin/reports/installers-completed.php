...
if (!empty($_REQUEST['searchDate'])){	
	...
	$where .= " AND tblJobTaskInstall.DateCompleted BETWEEN '" . date("Y-m-d", strtotime($dates[0])) . "' AND '" . date("Y-m-d", strtotime($dates[1] . '+1 day')) . "'";
}
...
<div id="print-content">
    ...
    <h1>
        <span id="pageTitle">Installer Jobs (completed)</span>
    ...
    <div class="table">
        ...
        <?php
			$query = "SELECT Count(tblJob.JobID) FROM tblJob INNER JOIN tblJobTaskInstall ON tblJob.JobID = tblJobTaskInstall.JobID $where AND tblJobTaskInstall.DateCompleted IS NOT NULL ORDER BY tblJob.JobID";
