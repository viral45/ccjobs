...
if (isset($_POST['action']))
{
	if ($_POST['action'] == "edit"){
		$scheduleid = isset($_POST['scheduleid']) ? $_POST['scheduleid'] : die('ERROR: Schedule ID not found.');

		if ($stmt = $mysqli->prepare("SELECT ScheduleID, UserID, JobID, Description, ScheduleDate, SortOrder, Notes FROM tblSchedule WHERE ScheduleID = ? LIMIT 1")) { 
			$stmt->bind_param('i', $scheduleid);
			$stmt->execute();
			$stmt->store_result();
			$stmt->bind_result($formscheduleid, $formuserid, $formjobid, $formdescription, $formscheduledate, $formsortorder, $formnotes);
			$stmt->fetch();
		}
	}
}
