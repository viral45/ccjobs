				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div >
							...
							<?php echo "<h3>WEEK " . $mondaydate . " to " . $saturdaydate . "</h3>"; ?>
								...
								<tbody>
									...
									<?php 
										foreach ($datearray as $day){ 
											...
											$schedulequery = "SELECT tblSchedule.ScheduleID, tblJob.JobAddress, tblJob.Builder, tblJob.JobID, tblSchedule.Description, tblSchedule.Notes FROM tblJob RIGHT JOIN tblSchedule ON tblJob.JobID = tblSchedule.JobID WHERE ScheduleDate = '" . date('Y-m-d',strtotime($day)) . "' AND UserID = '" . $_SESSION['user_id'] ."' ORDER BY SortOrder";
											...
											while($schedulerow = $scheduleresult->fetch_array()){
												...
												if (!empty($schedulerow['JobID']))
													echo '<br><a href="job.php?jobid=' . $schedulerow['JobID'] . '#installer" class="btn btn-primary btn-xs edit-btn">View Job</a>';
						...
						<a class='btn btn-primary week-btn' href="my-schedule.php?weekstart=<?php echo date('d-m-Y',strtotime("monday last week", strtotime($weekstart))) ?>">
							<span aria-hidden="true">&laquo;</span> Previous Week
						</a>

						<a class='btn btn-primary week-btn' href="my-schedule.php?weekstart=<?php echo date('d-m-Y',strtotime("monday next week", strtotime($weekstart))) ?>">
